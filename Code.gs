...
function getPillData(pillName) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (
      data[i][1] &&
      data[i][1].toLowerCase().trim() === pillName.toLowerCase().trim()
    ) {
      return {
...
function getAppointmentsByEmail(email) {
  const bookingSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Appointments"); // Replace "Appointments" with your actual sheet name if different
  const allData = bookingSheet.getDataRange().getValues();
  const filteredData = allData.filter(
    (row, index) =>
      index > 0 && row[1].trim().toLowerCase() === email.trim().toLowerCase()
  ); // Assuming email is in the second column (B), and skipping the header row
